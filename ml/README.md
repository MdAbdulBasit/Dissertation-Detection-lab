...
python3 scripts/triage_model.py     # xgboost is optional; the script skips that block without it
```
...
|---|---|---|---|---|---|
| Random Forest | GroupKFold by window | 0.844 | 0.724 | **0.784** | 0.973 |
| XGBoost | GroupKFold by window | 0.841 | 0.719 | **0.780** | 0.971 |
| Random Forest | LeaveOneTechniqueOut | 0.706 | 0.372 | **0.539** | 0.736 |
| XGBoost | LeaveOneTechniqueOut | 0.722 | 0.428 | **0.575** | 0.742 |

**A 0.004 difference within techniques, against a fold standard deviation of 0.071.** The two models are
indistinguishable. XGBoost is modestly better on unseen techniques (+0.036) but still fails there.

This is worth stating rather than hiding: **swapping the classifier changes nothing.** What moved the
numbers in this project was the feature representation and the leak sanitisation — removing harness
tokens cost ~0.01, removing rule identity cost 0.04. Model selection is where effort is conventionally
spent and it was the least productive thing available here.

### ⚠️ Confusion matrices — the error balance macro F1 was hiding

`evaluate()` computed a confusion matrix from the beginning and nothing ever printed it. Random Forest,
`B_no_rule`, GroupKFold, at the default 0.5 threshold:
...
**The model misses 24.5% of attacks and false-alarms on only 8.2% of benign activity.** For a triage
tool that is the *wrong way round*. A false positive costs an analyst a few minutes; a false negative is
an intrusion nobody looks at. Macro F1 0.784 treats the two as interchangeable and they are not.

The earlier claim in this file — *"0.958 precision at 0.755 recall is an operationally useful trade"* —
now reads differently. 0.755 recall means **471 missed attacks out of 1,919**. Defensible only if the
threshold is tuned, which is what the next section does.

### ⭐ Operating points — what a SOC would actually deploy

A model is not deployed at argmax F1. It is deployed where the miss rate is tolerable. Sweeping the
decision threshold on the same out-of-fold probabilities:

| Threshold | Alerts reviewed | % of queue | Attacks caught | **Missed** | Precision | Recall |
|---|---|---|---|---|---|---|
| 0.20 | 2,273 | 84.7% | 1,899 | **20** | 0.835 | 0.990 |
| 0.25 | 2,179 | 81.2% | 1,850 | 69 | 0.849 | 0.964 |
| 0.30 | 2,063 | 76.9% | 1,803 | 116 | 0.874 | 0.940 |
| 0.40 | 1,877 | 70.0% | 1,701 | 218 | 0.906 | 0.886 |
| **0.50 (default)** | 1,511 | 56.3% | 1,448 | **471** | 0.958 | 0.755 |
| 0.60 | 1,393 | 51.9% | 1,387 | 532 | 0.996 | 0.723 |
| 0.95 | 1,155 | 43.0% | 1,154 | 765 | 0.999 | 0.601 |

**Two defensible operating points:**

- **Miss-averse — threshold 0.20.** Review 84.7% of the queue, catch **99.0%** of attacks, miss 20.
  Workload reduction ~15%.
- **Load-averse — threshold 0.50.** Review 56.3%, but miss 471 attacks. **Not deployable in a real SOC.**

**The honest headline is that the achievable workload saving is about 15–20%, not an order of
magnitude.** Keeping recall above 95% requires reviewing roughly four alerts in five. That is a real
saving — it is not the transformation alert-triage papers usually imply, and this dataset does not
support a stronger claim.

Note the plateau between thresholds 0.55 and 0.75: the count reviewed barely moves (1,386–1,404). The
model is highly confident about one large cluster of alerts and the threshold has little purchase there.

> ⚠️ **All of this is measured on a 72/28 attack-to-benign mix.** A production SOC queue is
> overwhelmingly benign, so precision at every threshold would be substantially lower in deployment.
> These figures are an upper bound, not a forecast.
...
you get by escalating everything**.

⚠️ **But not at the default threshold.** At 0.5 the model misses 471 of 1,919 attacks. The usable
configuration is threshold ≈ 0.20, which catches 99.0% of attacks while cutting the queue by about 15%.
The comparison above is a statement about *ranking quality*, not a deployable configuration.
...
- No hyperparameter search. Defensible in hindsight: **XGBoost matched Random Forest to within 0.004**,
  so tuning was unlikely to be where the gains were. The comparison between feature sets, not peak
  performance, was the experiment.
- **The workload saving is ~15%, not an order of magnitude**, once the threshold is set somewhere a SOC
  could actually accept. Stated plainly because the operating-point table makes it impossible to imply
  otherwise.
- **The class balance flatters every precision figure here.** 72/28 attack-to-benign is an artefact of
  running attacks deliberately; a real queue is overwhelmingly benign. Treat all precision numbers as an
  upper bound.
